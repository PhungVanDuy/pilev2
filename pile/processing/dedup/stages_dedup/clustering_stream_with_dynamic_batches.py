...
class IterDataset:
    ...
    def calc_batch_indices(self,batch_size:int):
        batch_config_list = []
        for subset in self.len_range_offset_dict:
            if self.len_range_offset_dict[subset][1] / batch_size >= 1:
                for i in range(0,self.len_dict_offset[subset],batch_size):
                    if i+batch_size > self.len_range_offset_dict[subset][1]-self.len_range_offset_dict[subset][0]:
                        start_idx = i
                        end_idx = self.len_range_offset_dict[subset][1]-self.len_range_offset_dict[subset][0]
                    else:
                        start_idx = i
                    ...
                    batch_config = {
                        ...
                        "end" : end_idx,
                        "dataset" : subset,
                        "batch_size" : len(range(start_idx,end_idx)),

                    }
                    batch_config_list.append(batch_config)
    ...
    def get_range_optimized(self,start:int,end:int,dataset_path:str):
        if self.dataset_in_mem_name == dataset_path:
            self.logger.info(f"Dataset {dataset_path} is already in memory...")
            return self.dataset_in_mem.select(range(start,end))
        else:
            self.logger.info(f"Dataset {dataset_path} is not in memory...")
            self.dataset_in_mem_name = dataset_path
            self.dataset_in_mem = datasets.load_from_disk(dataset_path)
            return self.dataset_in_mem.select(range(start,end))
...
if __name__ == "__main__":
    def run(
        ...
        batch_config_list = embedded.calc_batch_indices(batch_size)
        first = embedded.get_range_optimized(0, 1,batch_config_list[0]["dataset"])[0] #Just get the first element.
        B = len(first['__signatures__'])        
        for table_idx in range(B):
            new_hash_table = defaultdict(set)
            for i in tqdm(
                range(len(batch_config_list)), dynamic_ncols=True, desc="Iterating MinHashes..."  # noqa: E501
            ):
                batch_config_curr = batch_config_list[i]
                batch = embedded.get_range_optimized(batch_config_curr["start"],batch_config_curr["end"],batch_config_curr["dataset"])
