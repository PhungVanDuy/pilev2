...
class IterDataset:
    ...
    def get_range(self, start_idx:int, end_idx:int):
        ...
        dataset_collator = []
        time_measures = {}
        time_measures['find_datasets_in_range'] = time.time()
        datasets_in_range, range_list_dataset = self.find_datasets_in_range(start_idx,end_idx)
        time_measures['find_datasets_in_range'] = time.time() - time_measures['find_datasets_in_range']
        time_measures['get_slicing_ind'] = time.time()
        slice_dict,dataset_to_load = self.get_slicing_ind(start_idx,end_idx)
        time_measures['get_slicing_ind'] = time.time() - time_measures['get_slicing_ind']
        time_measures['get_range'] = time.time()
        ...
        time_measures['get_range'] = time.time() - time_measures['get_range']
        print(time_measures)
...
if __name__ == "__main__":
    def run(
        ...
        batch_size: int = typer.Option(10000, help="batch size for deduping"),
                                                      ):
        global uf
        ...
        ds_size = sys.getsizeof(embedded) / 1024 / 1024 / 1024
        print(f"Size of embedded dataset is {ds_size} GB at loading time")
        print('output will be saved to the following files:')
        outputs = dict()
        for minhash_dataset_path in minhash_dataset_paths:
            print(minhash_dataset_path)
            dataset_name = minhash_dataset_path.split("/")[-1]
            folder_dir = os.path.join(f"{minhash_dataset_path[:-len(dataset_name)]}", "filter_idx")
            outputs[minhash_dataset_path] = os.path.join(folder_dir, f"{dataset_name}_filter_idx")
        ...
        for table_idx in range(B):
            ...
            for i in tqdm(
                ...
                ds_size = sys.getsizeof(embedded) / 1024 / 1024 / 1024
                print(f"Size of embedded dataset is {ds_size} GB at iteration {i}")
        ...
        for minhash_dataset_path in minhash_dataset_paths:
            ...
            final_data.save_to_disk(output, max_shard_size= "500MB")
